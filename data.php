...
//echo print_r($surveiItemList[0]['id']);
		...
		//echo print_r($arrQts);
		$noK = -1;
		$jml = $conn->query("SELECT COUNT(*) AS jml FROM survey_answers GROUP BY participant_id")->fetch_array(); //menghitung jumlah record dalam db
...
function updateSheet($data,$jml) //update sheet
{
	...
	//update data
	$cell = $jml+1; //cell nomor 1 (nomor 2 dari atas)
